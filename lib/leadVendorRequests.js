...
function buildVendorWhatsappMessage({
  ...
  const cleanRequirementSubject = cleanString(requirementSubject);

  const cleanRequirementText =
    cleanString(requirementText) || htmlToText(requirementHtml);

  /*
    Remove duplicated auto-generated lines from rich text.
    The structured details are already shown separately below.
  */
  const customRequirementLines = cleanRequirementText
    .split(/\r?\n/)
    .map(item => cleanString(item))
    .filter(Boolean)
    .filter(item => {
      ...
      return !(
        ...
        lower.startsWith("travel date:") ||
        lower.startsWith("travel dates:") ||
        lower.startsWith("pax:") ||
        lower.startsWith("guest:") ||
        lower.startsWith("guests:") ||
        lower.startsWith("nights") ||
        lower.startsWith("duration") ||
        lower.includes("please share your best costing") ||
        lower.includes("please share your best quotation") ||
        lower.includes("availability, inclusions, exclusions")
      );
    });
  ...
  const shouldShowSubject =
    cleanRequirementSubject &&
    cleanRequirementSubject.toLowerCase() !==
      "travel requirement for quotation";

  const detailRows = [
    vendorLeadReference ? `*Vendor Ref:* ${vendorLeadReference}` : "",
    shouldShowSubject ? `*Request:* ${cleanRequirementSubject}` : "",
    destinationName ? `*Destination:* ${destinationName}` : "",
    finalTravelDates ? `*Travel Dates:* ${finalTravelDates}` : "",
    finalPaxText ? `*Pax:* ${finalPaxText}` : "",
    expectedTatLabel ? `*Expected TAT:* ${expectedTatLabel}` : "",
    expectedReplyByText
      ? `*Reply Required By:* ${expectedReplyByText}`
      : ""
  ].filter(Boolean);
  ...
  const requestedDetails = [
    "Best net rates and availability",
    "Suitable hotel / service options, wherever applicable",
    "Inclusions and exclusions",
    "Payment terms",
    "Cancellation policy",
    "Rate validity",
    "Important operational notes, blackout dates, peak surcharges, or restrictions, if any"
  ];

  return [
    `Hello ${vendorGreetingName},`,

    "We request your support for the below travel requirement. Please review the details and share your best possible quotation.",

    [
      "*Vendor Requirement Brief*",
      "",
      detailRows.join("\n")
    ].join("\n"),

    customRequirementLines.length
      ? [
          "*Requirement Notes*",
          "",
          customRequirementLines.map(item => `- ${item}`).join("\n")
        ].join("\n")
      : "",
    ...
    [
      "*Please Share*",
      "",
      requestedDetails.map(item => `- ${item}`).join("\n")
    ].join("\n"),

    "Your timely support will help us shape a seamless and memorable travel experience for our partner’s client, bringing our promise of Realize the Experiance to life.",
